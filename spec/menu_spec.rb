...
RSpec.describe CheckList::Menu do
  ...
  context '.new' do
    it 'initalizes without error' do
      ...
      expect { subject.new(filepath) }.not_to raise_error
    end
  end
  ...
  context '.show_menu' do
    ...
    it 'leaves when 0' do
      ...
      expect(subject.new(filepath).show_menu).to eq 'good_bye'
    end
    ...
    it 'shows tasks if not zero' do
      ...
      expect(subject.new(filepath).show_menu).to eq 'show_tasks'
    end
    ...
    it 'calls itself if ' do
      ...
      expect(subject.new(filepath).show_menu).to eq 'show_menu'
    end
  end
  ...
  context '.rec_show_menu' do
    ...
    it { expect(subject.new(filepath).rec_show_menu).to eq 'show_menu' }
  end
  ...
  context '.get_list' do
    it{ expect(subject.new(filepath).get_list(1)).to eq filepath_lists['lists'][0] }
  end

  context '.good_bye' do
    it{ expect(subject.new(filepath).good_bye).to eq 'leave'}
  end
end